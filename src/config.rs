    ...
    let matches = App::new("Load tester")
        ...
        .arg(
            Arg::with_name("url")
                .short("u")
                .long("url")
                .help("Sets the URL to connect to (string)")
                .takes_value(true)
                .required(false),
        )
        .arg(
        ...
        .arg(
            Arg::with_name("duration")
                .short("d")
                .long("duration")
                .help("Sets the approx. duration for sending tps in seconds (u32)")
                .takes_value(true)
                .validator(|val| 
                    match val.parse::<u32>().is_ok() {
                        true => Ok(()),
        ...
        .arg(
            Arg::with_name("setup_not_required")
                ...
                .help("Indicates setup is not required")
                .takes_value(false) 
                .conflicts_with("tps"),
        )
        .get_matches();

    let tps = matches.value_of("tps").map(|val| val.parse::<u8>().unwrap()).unwrap();
    let url = matches.value_of("url").unwrap_or("ws://127.0.0.1:9944").to_string();
    let duration = matches.value_of("duration").map(|val| val.parse::<u32>().unwrap()).unwrap();
    let nonce = matches.value_of("nonce").map(|val| val.parse::<u32>().unwrap()).unwrap();
